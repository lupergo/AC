...
#SBATCH -o salida_v3_dynamic_O0_%j.out
...
RESULTADOS="resultados_v3_dynamic_O0.csv"
...
for cflags in $VALORES_CFLAGS; do
    for acumulacion in reduction critical; do
        ...
        echo "=== Compilando v3 con CFLAGS=${cflags}, schedule=dynamic, acumulacion=${acumulacion} ==="
        make v3 -B CFLAGS="${cflags}" CPPFLAGS="-DUSE_REDUCTION=${use_reduction} -DSCHED_KIND=dynamic" 2>&1

        for n in $VALORES_N; do
            for hilos in $VALORES_HILOS; do
                for rep in $(seq 1 $REPETICIONES); do
                    output=$(./v3 $n $hilos)
                    echo "v3,${n},${cflags},${hilos},dynamic,${acumulacion},${rep},${output}" >> "$RESULTADOS"
                done
                echo "  v3 | n=${n} | hilos=${hilos} | cflags=${cflags} | schedule=dynamic | acumulacion=${acumulacion} | completado"
            done
        done
    done
done
